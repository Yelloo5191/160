...
class OBJLoader {
        ...
        while ((line = lines[index++]) != null) {
            ...
            switch (command) {
                ...
                case "mtllib": // Read Material chunk
                    ...
                    if (!response.ok)
                        throw new Error(
                            `Could not load material "${path}". Is the .mtl file in the same folder as the .obj file?`
                        );
    ...
    async parseModel() {
        try {
            ...
            if (!result)
                throw new Error(`Something went wrong parsing the .obj file.`);
    ...
    parseFace(sp, materialName, vertices, reverse) {
        ...
        if (normal == null) {
            if (face.vIndices.length >= 4) {
                // If the face is a quadrilateral, calculate the normal using a combination of three other points
                let v3 = [
            ...
            if (normal == null) {
                // The normal could not be calculated, so the normal in the Y-axis direction is used.
                normal = [0.0, 1.0, 0.0];
            }
        }
        if (reverse) {
    ...
    getModelData() {
        ...
        return new DrawingInfo(
            new Float32Array(unpackedVerts),
            new Float32Array(unpackedNormals),
...
class LoaderUtils {
    constructor() {
        throw new Error(
            "LoaderUtils is a static class; do not instantiate it."
        );
    }
